/** Balance ItemsSeries API row (admin `balanceSync.ts`-ის ანალოგი) */
export type BalanceItemsSeriesApiRow = {
  uid: string;
  Item: string;
  Name: string;
  ValidUntil: string;
...
/** Balance → Mongo სინქის payload */
export type BalanceSyncProductPatch = Record<string, unknown>;
...
export function buildPriceByUuid(
  pricesRows: Record<string, unknown>[],
): Map<string, number> {
...
function balanceDiscountRuleActiveByPeriod(
  row: Record<string, unknown>,
): boolean {
...
function balanceDiscountRuleIsActiveFlag(
  row: Record<string, unknown>,
): boolean {
...
export function buildTaxationByUuid(
...
export function mapBalanceItemToProduct(
  ...
  return {
    ...
    unitOfMeasure:
      getStr(item, 'UnitOfMeasure', 'Unit', 'unitOfMeasure', 'UOM') ||
      undefined,
    productCode:
      getStr(item, 'Code', 'ProductCode', 'productCode') || undefined,
    barcode: getStr(item, 'Barcode', 'barcode', 'GTIN') || undefined,
    genericName: getStr(item, 'GenericName', 'genericName') || undefined,
    productNameBrand:
      getStr(item, 'ProductNameBrand', 'Brand', 'productNameBrand') ||
      undefined,
    ...
    documentNumber:
      getStr(item, 'DocumentNumber', 'documentNumber') || undefined,
    certificateNumber:
      getStr(item, 'CertificateNumber', 'certificateNumber') || undefined,
    expiryDate:
      getDate(item, 'ExpiryDate', 'expiryDate', 'Expiry') || undefined,
    activationDate:
      getDate(item, 'ActivationDate', 'activationDate') || undefined,
    transportStartDate:
      getDate(item, 'TransportStartDate', 'transportStartDate') || undefined,
...
export function stockLinesHaveSeriesUuid(
...
export function buildBalanceWarehouseNameByUuid(
 ...
 */
export function normalizeBalanceItemSeriesRows(
  data: unknown,
  ...
  for (const row of rows) {
    ...
    const suClean =
      (seriesRowUid ||
        (suFallback && suFallback !== NULL_GROUP_UID
          ? suFallback
          : undefined)) ??
      undefined;
...
export function itemsSeriesNumbersForTable(
  lines: BalanceItemSeriesLine[],
): string {
  if (lines.length === 0) return '—';
  const parts = [
    ...new Set(
      lines.map((l) => l.seriesNumber?.trim()).filter(Boolean) as string[],
    ),
  ];
  return parts.length > 0 ? parts.join('; ') : '—';
}
...
export function itemsSeriesValidUntilForTable(
  lines: BalanceItemSeriesLine[],
): string {
  if (lines.length === 0) return '—';
  const parts = [
    ...new Set(
      lines.map((l) => l.expiryDate?.trim()).filter(Boolean) as string[],
    ),
  ];
  return parts.length > 0 ? parts.join('; ') : '—';
}
...
export function mergeBalanceItemSeriesFromStocks(
  ...
  if (withSeries.length > 0) {
    return withSeries.map((sl) => {
      ...
      const meta = singleMeta ?? (k ? metaBySeriesRef.get(k) : undefined);
      return {
